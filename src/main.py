...
df1 = df_flights.filter(df_flights.origin.startswith("EH") & df_flights.destination.substr(1,2).isin(icao) & ~df_flights.origin.isin(["EHVK","EHDB","EHDP","EHGR","EHDS","EHND","EHMZ","EHDR","EHHV","EHLW","EHWO","EHOW"]) & (df_flights.origin != df_flights.destination))
...
df1.select(df1.origin, df1.destination, df1.time_difference, df1.firstseen, df1.filename).write.mode("append").csv("/user/s2484765/testing_project")

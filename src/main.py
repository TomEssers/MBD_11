...
# icao = spark.read.option("header", "true").option("delimiter",";").csv("./ICAO_europe.csv")
# plane_types = spark.read.option("header", "true").option("delimiter", ";").csv("./AIRCRAFTS.csv")

# # Extract the relevant column as a list
# icao = [row['ICAO'] for row in icao.collect()]
